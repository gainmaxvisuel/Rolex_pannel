...
function renderDeposits() {
  ...
  filteredDeposits.forEach(deposit => {
    ...
    /*
     * IMPORTANT :
     * On vérifie uniquement si la preuve existe.
     * Le statut du dépôt n'empêche jamais
    ...
    if (hasProof) {
      ...
      /*
       * Orange Money BF peut ne pas avoir
       * de capture d'écran.
       */
...
async function viewProof(depositId) {
  ...
  try {
     ...
     *
     * afin d'éviter le message générique
    ...
    console.log(
      "Réponse brute admin-deposit-proof:",
      rawText
    );
    ...
    try {
      ...
      throw new Error(
        `Réponse invalide de l'Edge Function (${response.status}).`
      );
    }


    console.log(
      "Réponse admin-deposit-proof:",
      data
    );
    ...
    /*
     * Ouverture de la preuve.
     *
     * L'image ou le PDF s'ouvre dans
     * un nouvel onglet.
     */
    ...
    if (!proofWindow) {
      ...
      showMessage(
        "Le navigateur bloque l'ouverture de la preuve. Autorisez les fenêtres pop-up.",
        "error"
      );
...
autoRefreshTimer = setInterval(
  () => {
    ...
    /*
     * Ne pas actualiser pendant
     * qu'une fenêtre de rejet est ouverte.
     */
